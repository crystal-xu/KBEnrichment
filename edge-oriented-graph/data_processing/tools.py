    ...
    for c in combs:
        ...
        for tr in true_rels:
            ...
            if list(set(tr.arg1).intersection(set(c[0]))) and list(set(tr.arg2).intersection(set(c[1]))):
                ...
                if uents[c[0]][0].type in type1 and uents[c[1]][0].type in type2:
                    pairs[(c[0], c[1])] = \
            ...
            # BA existing relation
                ...
                #for t1, t2 in zip(type1, type2):
                if uents[c[1]][0].type in type1 and uents[c[0]][0].type in type2:
                    pairs[(c[1], c[0])] = \
        ...
        if not_found_rels != total_rels:
            if (c[0], c[1]) in pairs and (c[1], c[0]) not in pairs:
                pairs[(c[1], c[0])] = \
                    PairStruct(a1.pmid, '1:NR:2', c[1], c[0], 'R2L', cross_res, (a2, a1))
            if (c[1], c[0]) in pairs and (c[0], c[1]) not in pairs:
                pairs[(c[0], c[1])] = \
                    PairStruct(a1.pmid, '1:NR:2', c[0], c[1], 'L2R', cross_res, (a1, a2))
