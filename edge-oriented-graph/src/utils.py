...
# Save checkpoint (by Yuwei Xu)
# Refer to the implementations of NLU+ CW2
def save_checkpoint(save_interval, model_folder, model, optimizer, epoch, best_score, train_res, test_res, best_epoch, best=False):
    ...
    if best:
        torch.save(state_dict, os.path.join(os.path.join(model_folder, 'checkpoint_best.pt')))
    else:
        if epoch % save_interval == 0:
            torch.save(state_dict, os.path.join(model_folder, 'checkpoint{}.pt'.format(epoch)))
            print("Saving checkpoint {}".format(os.path.join(model_folder, 'checkpoint{}.pt'.format(epoch))))
        if last_epoch < epoch:
            torch.save(state_dict, os.path.join(model_folder, 'checkpoint_last.pt'))
...
def save_model(model_folder, trainer, loader):
    ...
    torch.save(trainer.state_dict(), os.path.join(model_folder, 're.model'))
