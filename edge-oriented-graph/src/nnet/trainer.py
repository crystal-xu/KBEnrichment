...
from utils import print_results, write_preds, write_errors, print_options, load_checkpoint, save_checkpoint, save_model
...
class Trainer:
    def __init__(self, loader, params, data, model_folder):
        ...
        self.save_interval = 50 # the interval of saving the checkpoint
        self.accumulation_steps = 32 # gradient accumulation steps
    ...
    def run(self):
        ...
        state_dict = load_checkpoint(self.model_folder, self.model, self.optimizer)
        if state_dict is not None:
            last_epoch = state_dict['last_epoch']
            if 'best_epoch' in state_dict:
                self.best_epoch = state_dict['best_epoch']
            self.train_res = state_dict['train_res']
            self.test_res = state_dict['test_res']
            self.best_score = state_dict['best_score']
        else:
            last_epoch = -1
        for epoch in range(last_epoch + 1, self.epoch+1):
            ...
            if epoch % self.save_interval == 0:
                save_checkpoint(self.save_interval, self.model_folder, self.model, self.optimizer, epoch,
                                self.best_score, self.train_res, self.test_res, self.best_epoch)
    ...
    def epoch_checking(self, epoch):
        ...
        if self.test_res['score'][-1] > self.best_score:  # improvement
            ...
            save_checkpoint(self.save_interval, self.model_folder, self.model, self.optimizer, epoch, self.best_score, self.train_res, self.test_res, self.best_epoch, True)
        elif self.es:
            self.cur_patience += 1
