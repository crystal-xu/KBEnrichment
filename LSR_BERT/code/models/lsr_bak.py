...
class LSR(nn.Module):
    def __init__(self, config):
        ...
        self.input_size = config.data_word_vec.shape[1] + config.coref_size + config.entity_type_size #+ char_hidden
        self.bert_hidden_size = 768
        self.bert_input_size = self.bert_hidden_size
        if self.use_coreference:
            self.bert_input_size += config.coref_size
        if self.use_entity_type:
            self.bert_input_size += config.entity_type_size


        self.bert = BertModel.from_pretrained('bert-base-uncased')

        #self.linear_re = nn.Linear(hidden_size * 2,  hidden_size)
        self.linear_re = nn.Linear(self.bert_input_size, hidden_size)
        ...
        self.rnn_sent = Encoder(self.input_size, hidden_size, config.dropout_emb, config.dropout_rate)
        #self.bert_sent = BertEncoder()
    ...
    def doc_encoder_bert(self, context_idxs, context_masks, context_starts, context_seg_bert, context_seg):
        """
        :param sent: sent emb
        :param context_seg: segmentation mask for sentences in a document
        :return:
        """
        batch_size = context_seg_bert.shape[0]
        docs_emb = [] # sentence embedding
        docs_len = []
        #sents_emb = []
        ...
        for batch_no in range(batch_size):
            ...
            sent_origin_lens = []
            pre_index = 0
            sent_origin_index = ((context_seg[batch_no] == 1).nonzero()).squeeze(-1).tolist() # array of st
            for i, index in enumerate(sent_origin_index):
                if i != 0:
                    if i == 1:
                        sent_origin_lens.append(index - pre_index + 1)
                    else:
                        sent_origin_lens.append(index - pre_index)
                pre_index = index
            #print("context_seg_bert[batch_no]: ", context_seg_bert[batch_no])
            sent_index = ((context_seg_bert[batch_no] == 1).nonzero()).squeeze(-1).tolist() # array of start point for sentences in a document
            pre_index = 0
            for i, index in enumerate(sent_index):
                if i != 0:
                    if i == 1:
                        sent_list.append(context_idxs[batch_no][pre_index:index+1])
                        mask_list.append(context_masks[batch_no][pre_index:index+1])
                        start_list.append(context_starts[batch_no][pre_index:index+1])
                        sent_lens.append(index - pre_index + 1)
                    else:
                        sent_list.append(context_idxs[batch_no][pre_index+1:index+1])
                        mask_list.append(context_masks[batch_no][pre_index+1:index+1])
                        start_list.append(context_starts[batch_no][pre_index+1:index+1])
                        sent_lens.append(index - pre_index)
                pre_index = index
            # print("sent_list: ", sent_list)
            # print(sent_list)
            sents = pad_sequence(sent_list).permute(1,0)
            # print(sents)
            masks = pad_sequence(mask_list).permute(1,0)
            starts = pad_sequence(start_list).permute(1,0)
            # print("sents: ", sents.size())
            # print("masks: ", masks.size())
            # print("starts: ", starts.size())
            #sent_lens_t = torch.LongTensor(sent_lens).cuda()
            docs_len.append(sent_lens)
            sents_output = self.bert(sents, attention_mask=masks)[0]
            # print("sents_output 1: ", sents_output.size())
            sents_output = [layer[starts.nonzero().squeeze(1)]
                              for layer, starts in zip(sents_output, starts)]
            # print(sents_output)
            # print("sents_output 2: ", len(sents_output), len(sents_output[0]), len(sents_output[0][0]))
            sents_output = pad_sequence(sents_output, batch_first=True, padding_value=-1)
            # print(sents_output)
            # print("sents_output 3: ", len(sents_output))
            sents_output = torch.nn.functional.pad(sents_output, (0, 0, 0, sents.size(-1) - sents_output.size(-2)))
            # print(sents_output)
            # print("sents_output 4: ", len(sents_output))

            doc_emb = None
            for i, (sen_len, emb) in enumerate(zip(sent_origin_lens, sents_output)):
                if i == 0:
                    doc_emb = emb[:sen_len]
                else:
                    doc_emb = torch.cat([doc_emb, emb[:sen_len]], dim = 0)
    ...
    def forward(self, context_idxs, pos, context_ner, context_char_idxs, input_lengths,
                h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h, context_seg, mention_node_position, entity_position,
                mention_node_sent_num, all_node_num, entity_num_list, sdp_pos, sdp_num_list,
                sent_idxs, sent_lengths, reverse_sent_idxs, context_masks, context_starts, context_seg_bert):
        """
        ...
        docs_rep = self.doc_encoder_bert(context_idxs, context_masks, context_starts, context_seg_bert, context_seg)
        if self.use_coreference:
            #print(docs_rep.size(), self.coref_embed(pos).size())
            coref_rep = self.coref_embed(pos)[:, :docs_rep.shape[1], :]
            docs_rep = torch.cat([docs_rep, coref_rep], dim=-1)

        if self.use_entity_type:
            ent_rep = self.ner_emb(context_ner)[:, :docs_rep.shape[1], :]
            docs_rep = torch.cat([docs_rep, ent_rep], dim=-1)
        max_doc_len = docs_rep.shape[1]
        #context_output = self.dropout_rate(torch.relu(self.linear_re(docs_rep)))
        context_output = self.linear_re(docs_rep)
        #print("context_output 1: ", context_output.size())
        ...
        # print("h_mapping 1: ", h_mapping.size())
        # print("h_mapping 2: ", h_mapping[:, :, :max_doc_len].size())
        # print("context_output 2: ", context_output.size())

        start_re_output = torch.matmul(h_mapping[:, :, :max_doc_len].float(), context_output) # aggregation
        end_re_output = torch.matmul(t_mapping[:, :, :max_doc_len].float(), context_output) # aggregation
