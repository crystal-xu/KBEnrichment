...
parser.add_argument('--in_path', type = str, default =  "../data_3050")
parser.add_argument('--out_path', type = str, default = "prepro_data_3050")
...
def sents_2_idx(sents, word2id):
    ...
    sents_idx = []
    start_idx = 0
    for i, sent in enumerate(sents[:MAX_SENT_NUM]):
...
# Modify by Yuwei
# Refer to the implementations: https://github.com/hongwang600/DocRed/tree/master
def Init(data_file_name, rel2id, max_length = 512, is_training = True, suffix=''):
    ...
    sen_seg_bert = np.zeros((sen_tot, max_length), dtype= np.int16)
    ...
    for i in range(len(ori_data)):
        ...
        for idx, sent in enumerate(item['sents']):
            tokens, masks, starts = bert.subword_tokenize_to_ids(sent)
            # print(bert_token[i])
            tokens_len = np.count_nonzero(tokens[0])
            if prev_pos == 0:
                bert_token[i][0:tokens_len] = tokens[0][:tokens_len]
                bert_mask[i][0:tokens_len] = masks[0][:tokens_len]
                bert_starts[i][0:tokens_len] = starts[0][:tokens_len]
                prev_pos += tokens_len
            else:
                if prev_pos >= 512:
                    continue
                if prev_pos + tokens_len > 512:
                    bert_token[i][prev_pos:512] = tokens[0][:512-prev_pos]
                    bert_mask[i][prev_pos:512] = masks[0][:512-prev_pos]
                    bert_starts[i][prev_pos:512] = starts[0][:512-prev_pos]
                else:
                    bert_token[i][prev_pos:prev_pos + tokens_len] = tokens[0][:tokens_len]
                    bert_mask[i][prev_pos:prev_pos + tokens_len] = masks[0][:tokens_len]
                    bert_starts[i][prev_pos:prev_pos + tokens_len] = starts[0][:tokens_len]
                prev_pos += tokens_len
            sen_seg_bert[i][np.count_nonzero(bert_token[i]) - 1] = 1
    ...
    np.save(os.path.join(out_path, name_prefix + suffix + '_seg_bert.npy'), sen_seg_bert)
